...
class BooksStorageUpdateTestCase(TestCase):
    ...
    def test_create_or_raise_if_exists_raises_error(self):
        existing_book = self.books[0]

        with pytest.raises(BookAlreadyExistsError) as exc_info:
            storage.create_or_raise_if_exists(existing_book)

        self.assertEqual(existing_book.slug, exc_info.value.slug)